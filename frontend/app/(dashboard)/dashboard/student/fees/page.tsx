...
const mockFees: FeeRecord[] = [
  { id: "1", amount: 50000, paidAmount: 40000, dueDate: "2026-12-28", status: "PARTIAL", description: "Term 1 Tuition" },
  { id: "2", amount: 15000, paidAmount: 0, dueDate: "2026-11-15", status: "PENDING", description: "Transport Fee" },
  { id: "3", amount: 8000, paidAmount: 0, dueDate: "2026-12-01", status: "OVERDUE", description: "Exam Fee" },
];
...
export default function StudentFeesPage() {
  ...
  const filteredFees = mockFees.filter((fee) => {
    const matchesSearch = fee.description.toLowerCase().includes(search.toLowerCase());
    ...
    <DashboardShell role="STUDENT">
      <div className="mb-8">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm text-slate-500">View and pay your fees</p>
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">My Fees</h2>
          </div>
        </div>
      </div>
              ...
              {filteredFees.map((fee) => (
                <tr key={fee.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4">
                    <div className="font-medium text-slate-900">{fee.description}</div>
                  </td>
